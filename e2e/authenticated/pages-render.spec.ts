...
import { expect, test, type Tenant } from "../tenants";
 ...
 *
 * Business pages are opened as business A's full user, with the records setup/tenants.php seeds;
 * server administration pages as the super admin.
 */
/** `fixme`: a note on a known bug that breaks the page, which marks its test test.fixme until it's fixed */
type PageCase = { url: (t: Tenant) => string; fixme?: string };

const businessPages: PageCase[] = [
  { url: () => "/" },
...
const placeholders = new Proxy({} as Tenant, {
...
async function problemsOpening(page: Page, url: string) {
