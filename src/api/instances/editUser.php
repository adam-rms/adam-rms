...
//Both the membership and the new role must be in this business
...
if (!$DBLIB->getOne("userInstances", ["userInstances.userInstances_id"])) finish(false, ["code" => "EDIT-USER-TO-INSTANCE-FAIL", "message"=> "Could not edit user in Business"]);
if (!$bCMS->positionIsInInstance($_POST['position'], $AUTH->data['instance']['instances_id'])) finish(false, ["code" => "EDIT-USER-TO-INSTANCE-FAIL", "message"=> "Could not edit user in Business"]);
$DBLIB->where("userInstances.userInstances_id", $_POST['userinstanceid']);
$DBLIB->where("userInstances.userInstances_deleted", 0);
