...
- `authenticated/pages-render.spec.ts` opens every page in a browser and fails on a PHP fatal error, the 404 page or an uncaught JavaScript error. Add new pages to its lists. Browser tests get CDN files from a local cache (`e2e/cdn.ts`) and can't reach any other outside site.
