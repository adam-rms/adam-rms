            ...
            if ($tokencheckresult != null) {
                if ((strtotime($tokencheckresult["authTokens_created"]) + (1 * 12 * (3600 * 1000))) < time()) {
                    ...
                    if ($this->data == null) {
                        ...
                        if (in_array(21, $this->permissions) and $this->data['users_selectedInstanceID'] != null and !in_array($this->data['users_selectedInstanceID'],$this->data['instance_ids'])) {
                            //They're assigned to an instance they are not in - so we need to download that instance as well for them
                            $DBLIB->where("instances.instances_deleted", 0);
                            $DBLIB->where("instances_id", $this->data['users_selectedInstanceID']);
                            ...
                            array_push($this->data['instance_ids'], $this->data['users_selectedInstanceID']);
                            $this->data['instance'] = $instance;
                            $this->data['instances'][] = $instance;
                        } elseif ($this->data['users_selectedInstanceID'] != null) $this->setInstance($this->data['users_selectedInstanceID']); //Set instance normally
