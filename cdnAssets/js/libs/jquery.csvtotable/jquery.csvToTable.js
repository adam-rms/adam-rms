/**
HEAVILY MODIFIED BY JAMES BITHELL - OCTOBER 2015 TO SUIT OUR PURPOSES!
 * CSV to Table plugin
	...
	*/
	...
	$.fn.CSVToTable = function(csvFile, options) {
		...
		return this.each(function() {
			...
			var tableHTML = '<table id="' + $(this).attr('id') + '_table" class="' + options.tableClass + '">';
			...
			$.each(lines, function(lineCount, line) {
				if ((lineCount == 0) && (typeof(options.headers) != 'undefined')) {
					...
					$.each(headers, function(headerCount, header) {
						tableHTML += '<th id="' + header[1] + '" data-override="' + header[1] + '" class="' + options.thClass + '">' + header[0] + '</th>';
					});
